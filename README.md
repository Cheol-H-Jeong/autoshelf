...
When Anthropic planning is enabled, autoshelf sends each file brief as a compact natural-language record with the immediate parent folder, full ancestor chain, a derived `meaningful_parent_hint`, duplicate-group size, and near-duplicate hints for text variants. That keeps business anchors from staging folders like `clients/acme/`, `receipts/`, or `강의자료/` visible during classification without making generic intake folders dominate the taxonomy, and it lets the planner keep versioned drafts of the same document together instead of scattering them across weak intake folders. The Anthropic prompt prefix is bundled into one cacheable system block with the planner rules, folder guide, and few-shot examples, so repeated chunked runs reuse the expensive shared instructions instead of paying for them on every request. After the initial assignment pass, autoshelf now runs a full-plan review step that can collapse weak workflow folders like `proposals/` back into stronger business parents such as a repeated client folder, and it rewrites the per-file summary into a folder rationale that shows up in the GUI and manifest.

The offline FakeLLM path now uses the same ancestry signals instead of relying on extension-only buckets. It also detects near-duplicate text variants with local shingled Jaccard matching, so a weak OCR extract or lightly edited copy can inherit the stronger classification chosen for its closest sibling. That means local-first runs can keep customer folders like `clients/acme/`, recognize finance-heavy docs such as invoices and receipts, keep adjacent document versions together, and split screenshot-heavy image imports into a dedicated bucket without calling the network.
...
For machine consumers, add `--progress json` before the subcommand to emit JSONL events on stdout. Each run now starts with a `command` event carrying `status: "started"`, streams `progress` events, emits a final `command` lifecycle event with `status: "completed"` or `status: "failed"` plus the exit code, and finishes with one `result` line when the command returns a payload. Unexpected crashes emit a structured `error` event instead of forcing wrappers to scrape stderr.
