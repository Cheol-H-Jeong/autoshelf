...
def parse_archive(path: Path, max_head_chars: int = 2000) -> ParsedContext:
    ...
    try:
        if suffix == ".zip":
            ...
            return ParsedContext(title=path.stem, head_text="", extra_meta={"parser": "unsupported"})
    except Exception:
...
PARSER_SPEC = ParserSpec(name="archive", suffixes=(".zip", ".tar", ".gz", ".tgz", ".7z"), parse=parse_archive)
